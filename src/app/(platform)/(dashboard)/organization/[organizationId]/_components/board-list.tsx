...
export const BoardList = async () => {
  ...
  const availableBoard = await getAvailableCount();
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
        {boards.map((board) => (
          <Link
            ...
            className="group relative aspect-video h-full overflow-hidden rounded-sm bg-sky-700 bg-cover bg-center bg-no-repeat p-2"
          >
          ...
          >
            ...
            {isPro ? (
              ...
              <span className="text-xs">
                {MAX_FREE_BOARDS - availableBoard} remaining
              </span>
            )}
            <Hint
